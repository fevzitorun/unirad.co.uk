...
• Self-referral available for selected scans — our clinical team assesses each patient's suitability before confirming their booking
...
function unirad_ai_handle_callback() {
    ...
    $body   = '<div style="font-family:Arial,sans-serif;font-size:14px;color:#0d1f3c;max-width:500px">'
            ...
            . '<p style="margin:0 0 10px;background:#f0faf9;padding:8px 10px;border-radius:6px;font-weight:bold;color:#00a896">Please call ' . esc_html( $name ?: 'the patient' ) . ' at ' . esc_html( $phone ) . '</p>'
...
.uai-lead-field{margin-bottom:5px;}
#uai-lead-in,#uai-lead-name{width:100%;box-sizing:border-box;border:1px solid rgba(0,168,150,.25);border-radius:18px;padding:6px 11px;font-size:11.5px;font-family:inherit;outline:none;color:#0b2240;}
#uai-lead-in:focus,#uai-lead-name:focus{border-color:#00a896;}
#uai-lead-time{width:100%;box-sizing:border-box;border:1px solid rgba(0,168,150,.25);border-radius:18px;padding:6px 11px;font-size:11.5px;font-family:inherit;outline:none;color:#0b2240;background:#fff;cursor:pointer;}
#uai-lead-go{width:100%;background:#00a896;color:#fff;border:none;border-radius:18px;padding:7px 13px;font-size:11.5px;font-weight:700;cursor:pointer;margin-top:2px;}
...
<div id="uai">
  ...
  <div id="uai-win" role="dialog" aria-label="MRI Booking Assistant">
    ...
    <div id="uai-lead">
      ...
      <div class="uai-lead-field"><input id="uai-lead-name" type="text" placeholder="Your name" autocomplete="name"></div>
      <div class="uai-lead-field"><input id="uai-lead-in" type="tel" placeholder="Your phone number" autocomplete="tel"></div>
      <div class="uai-lead-field"><select id="uai-lead-time"><option value="">Best time to call</option><option value="Morning (9am–12pm)">Morning (9am–12pm)</option><option value="Afternoon (12pm–5pm)">Afternoon (12pm–5pm)</option><option value="Evening (5pm–7pm)">Evening (5pm–7pm)</option></select></div>
      <button id="uai-lead-go">Request a call</button>
    </div>
...
function greet(){
  greeted = true;
  addMsg('b', "Hi there! I'm Aria, Unirad's MRI assistant 👋\n\nI can help you find the right scan, explain what to expect, and get you booked in — often **same week**, from £290. Self-referral is available for selected scans — our clinical team will guide you through the process.\n\nWhat body area are you looking to scan?");
}
...
newBtn.addEventListener('click', function(){
  ...
  lead.innerHTML = '<div id="uai-lead-p">Want us to call you about booking?</div>'
    + '<div class="uai-lead-field"><input id="uai-lead-name" type="text" placeholder="Your name" autocomplete="name"></div>'
    + '<div class="uai-lead-field"><input id="uai-lead-in" type="tel" placeholder="Your phone number" autocomplete="tel"></div>'
    + '<div class="uai-lead-field"><select id="uai-lead-time"><option value="">Best time to call</option><option value="Morning (9am–12pm)">Morning (9am–12pm)</option><option value="Afternoon (12pm–5pm)">Afternoon (12pm–5pm)</option><option value="Evening (5pm–7pm)">Evening (5pm–7pm)</option></select></div>'
    + '<button id="uai-lead-go">Request a call</button>';
...
function bindLead(){
  leadGo.addEventListener('click', function(){
    ...
    var name  = (document.getElementById('uai-lead-name') || {value:''}).value.trim();
    var ptime = (document.getElementById('uai-lead-time') || {value:''}).value;
    ...
    fd.append('name',   name);
    fd.append('note',   'AI Chat Callback' + (ptime ? ' — Best time: ' + ptime : ''));
    fetch(AJAX, {method:'POST', body:fd}).catch(function(){});
    ...
    addMsg('b', "Great" + (name ? ", " + name.split(" ")[0] : "") + " — we'll give you a call" + (ptime ? " " + ptime.toLowerCase() : " shortly") + ". You can also book directly at https://unirad.co.uk/book-your-scan/ — same-week appointments available.");
  });
}
bindLead();
